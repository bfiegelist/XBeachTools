function XBGenfun(ID, Ho, T,wavemodel,quality)
...
if convertCharsToStrings(wavemodel) == "surfbeat"
    ...
    elseif quality == "low"
        ...
        if Profile(prof).Xshore(1)==0
            [xgr zgr] = xb_grid_xgrid((Profile(prof).Xshore), (Profile(prof).Prof),'dxmax',0.33,'dxmin',0.1);
        else
            [xgr zgr] = xb_grid_xgrid(flip(Profile(prof).Xshore), flip(Profile(prof).Prof),'dxmax',0.33,'dxmin',0.1);
        end
         xbi = xb_generate_model( ...
                 'bathy',   { 'x', xgr, 'z', zgr},...
                 'waves',   { 'Hm0', Ho, 'Tp', T,'duration',duration}, ...
